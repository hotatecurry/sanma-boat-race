...
<style>
  * { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  body { display:flex; flex-direction:column; align-items:center; justify-content:center;
    ...
    gap:10px; padding:8px; touch-action:none; user-select:none; -webkit-touch-callout:none; }
...
// ==== モバイル：連打中の誤ズーム（ダブルタップ拡大／ピンチ）を抑止 ====
// iOS Safari は user-scalable=no を無視するため JS でジェスチャを止める
['gesturestart','gesturechange','gestureend'].forEach(ev=>
  document.addEventListener(ev, e=>e.preventDefault(), {passive:false}));
let _lastTapTs=0;
document.addEventListener('touchend', e=>{
  const now=Date.now();
  if(now-_lastTapTs<=350) e.preventDefault();   // 直前タップから350ms以内＝ダブルタップ拡大を無効化
  _lastTapTs=now;
}, {passive:false});
